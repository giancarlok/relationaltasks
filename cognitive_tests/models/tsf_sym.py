...
class Symmetric_Self_Attention(nn.Module):
    def __init__(self, dim, ffn_dim, nheads=4):
        super(Symmetric_Self_Attention, self).__init__()
        ...
        self.symmetric_query_key_net = nn.Linear(dim, dim)
    ...
    def forward(self, x):
        ...
        q = self.symmetric_query_key_net(x).reshape(bsz, n, self.nheads, self.head_dim)
        q = q.permute(0,2,1,3) / np.sqrt(self.head_dim)
        k = self.symmetric_query_key_net(x).reshape(bsz, n, self.nheads, self.head_dim)
        k = k.permute(0,2,3,1)
...
class Model(nn.Module):
    def __init__(self, task_gen, args):
        ...
        self.transformer_encoder = Symmetric_Self_Attention(self.z_size, self.dim_ff, self.n_heads)
