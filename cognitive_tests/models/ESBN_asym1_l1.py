...
class Model(nn.Module):
	def __init__(self, task_gen, args):
		...
		if self.freeze_encoder:
			...
			num_params = sum(p.numel() for p in encoder_model.parameters() if p.requires_grad)
			print("numer of params before freezing", num_params)
			for param in encoder_model.parameters():
				print("freezing params")
				param.requires_grad = False

			num_params = sum(p.numel() for p in encoder_model.parameters() if p.requires_grad)
			print("numer of params after freezing", num_params)
		...
		self.w1 = nn.Linear(self.z_size, self.z_size)
		self.w2 = nn.Linear(self.z_size, self.z_size)
	...
	def forward(self, x_seq, device):
		...
		z_seq1 = self.w1(z_seq)
		z_seq2 = self.w2(z_seq)
		l1_penalty = sum(p.abs().sum()
						 for p in self.w1.parameters())
		l1_penalty = l1_penalty + sum(p.abs().sum()
						 for p in self.w2.parameters())
		if self.contextnorm:
			z_seq_all_seg1 = []
			z_seq_all_seg2 = []
			for seg in range(len(self.task_seg)):
				z_seq_all_seg1.append(self.apply_context_norm(z_seq1[:,self.task_seg[seg],:]))
				z_seq_all_seg2.append(self.apply_context_norm(z_seq2[:, self.task_seg[seg], :]))
			z_seq1 = torch.cat(z_seq_all_seg1, dim=1)
			z_seq2 = torch.cat(z_seq_all_seg2, dim=1)
		...
		for t in range(x_seq.shape[1] + 1):
			# Image embedding
			if t == x_seq.shape[1]:
				z1_t = torch.zeros(x_seq.shape[0], 1, self.z_size).to(device)
				z2_t = torch.zeros(x_seq.shape[0], 1, self.z_size).to(device)
			else:
				z1_t = z_seq1[:,t,:].unsqueeze(1)
				z2_t = z_seq2[:, t, :].unsqueeze(1)
			# Controller
			# LSTM
			...
			if t == 0:
				...
				# Read key
				w_k = self.softmax((z2_t * M_v).sum(dim=2))
				c_k = self.sigmoid(((z2_t * M_v).sum(dim=2) * self.confidence_gain) + self.confidence_bias)
				# print("w_k", w_k.shape)
				# print("c_k", c_k.shape)
			...
			if t == 0:
				...
				M_v = torch.cat([M_v, z1_t], dim=1)
		return y_pred_linear, y_pred, None, l1_penalty
