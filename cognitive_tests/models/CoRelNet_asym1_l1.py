...
class Model(nn.Module):
	...
	def forward(self, x_seq, device):
		...
		s_seq1 = self.w1(s_seq)
		s_seq2 = self.w2(s_seq)
		...
		for seg in range(len(self.task_seg)):
			z_seq_all_seg1.append(self.apply_context_norm(s_seq1[:, self.task_seg[seg], :]))
			z_seq_all_seg2.append(self.apply_context_norm(s_seq2[:, self.task_seg[seg], :]))
		z_seq1 = torch.cat(z_seq_all_seg1, dim=1)
		z_seq2 = torch.cat(z_seq_all_seg2, dim=1)
		R_s = torch.matmul(z_seq1, z_seq2.transpose(2, 1))
