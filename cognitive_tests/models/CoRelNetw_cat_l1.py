...
class Model(nn.Module):
	def __init__(self, task_gen, args):
		...
		self.w = nn.Linear(self.z_size, self.z_size)
		...
		self.final = nn.Sequential(nn.Linear(self.matrix_size+self.task_dim*self.z_size, 256), nn.ReLU(), nn.Linear(256,task_gen.y_dim))
	...
	def forward(self, x_seq, device):
		...
		# x1 = s_seq.unsqueeze(2)
		# x2 = s_seq.unsqueeze(1)
		# x1 = x1.repeat(1, 1, s_seq.shape[1], 1)
		# x2 = x2.repeat(1, s_seq.shape[1], 1, 1)
		# concat_matrix = torch.cat([x1, x2], dim=3)
		...
		if self.reduced:
			...
			for i in range(1, R_s.shape[1]):
				...
				#c = torch.cat([c, concat_matrix[:, i, :i + 1, :]], dim=1)
			R_s = b
			#concat_matrix = c
		R_save = R_s.clone()
		R_s = R_s.view(R_s.shape[0], self.matrix_size, 1)
		x_s = s_seq.view(s_seq.shape[0], s_seq.shape[1]*s_seq.shape[2])
		R_s= R_s.squeeze()

		R_s = torch.cat([R_s,x_s], dim=1)
		y_pred_linear = self.final(R_s)
		y_pred = y_pred_linear.argmax(1)
		return y_pred_linear, y_pred, R_save, l1_penalty
