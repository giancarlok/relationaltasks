...
def get_sample(batch_size, prob, shapes, colours):
	...
	x=np.concatenate([x_aaa_bcb, x_aaa_cbb, x_aba5,x_abb5, x_aba6, x_abb6], axis=0)
	x = np.reshape(x, [-1])

	x = np.reshape(shapes[x], (6*batch_size, -1, 3, 32, 32))

	y=np.concatenate([bern_aaa_bcb, bern_aaa_cbb, bern_aba5, bern_abb5, bern_aba6, bern_abb6], axis=0)[:,0]
