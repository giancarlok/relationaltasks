...
def check(x,y):
	...
	save_image(img, "RMTS_sc_colours.png")
...
def get_sample(batch_size, prob, shapes, colours):
	...
	x_shapes = np.array([np.random.choice(len(shapes), size=6, replace=True) for _ in range(batch_size)])
	x_same = np.stack([np.random.choice(len(colours), size=4, replace=False) for _ in range(same)],axis=0)
	...
	#x_diff = colours[x_diff]


	x_c=np.concatenate([x_same, x_diff], axis=0)
	x_shapes = np.reshape(x_shapes, [-1])
	x_shapes = np.reshape(shapes[x_shapes], (batch_size, -1, 3, 32, 32))
