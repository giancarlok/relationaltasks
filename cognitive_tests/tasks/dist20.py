...
def get_sample(batch_size, prob, shapes, colours):
	...
	# pick 4 shapes
	obj= np.stack([np.random.choice(len(shapes), size=21, replace=False) for _ in range(batch_size)],axis=0)
	...
	perm = np.stack([np.random.choice(20, size=20, replace=False) for _ in range(batch_size)],axis=0)
	...
	perm2 = np.stack([np.random.choice(21, size=21, replace=False) for _ in range(batch_size)], axis=0)
...
def get_sample(batch_size, prob, shapes, colours):
	...
	diff = batch_size-same
	# pick 4 shapes
	obj= np.stack([np.random.choice(len(colours), size=21, replace=False) for _ in range(batch_size)],axis=0)
	# take the first 3 out of those chosen shapes for positions 1,2 and 3.
	obj_list=[]
	for i in range(20):
		obj_list.append(obj[:,i])
