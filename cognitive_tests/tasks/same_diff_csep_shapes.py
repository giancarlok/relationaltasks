...
def check(x,y):
    ...
    #print(img.shape)
    save_image(img, "same_diff_csep_shapes.png")
...
def get_sample(batch_size, prob, shapes, colours):
    ...
    x_col = np.random.choice(len(colours), size=(batch_size, 2), replace=True)
    x_same = np.random.choice(len(shapes), size=(same, 1), replace=True)
    ...
    x_same = np.reshape(shapes[x_same], (same, -1, 3, h, w))
    x_diff = np.reshape(x_diff, [-1])
    x_diff = np.reshape(shapes[x_diff], (diff, -1, 3, h, w))
    x_c = np.reshape(x_col, [-1])
    x_c = torch.Tensor(np.reshape(colours[x_c], (batch_size, -1, 3, h, w)))

    y = np.zeros(batch_size)
    y[-diff:] = 1
    x = torch.Tensor(np.concatenate([x_same, x_diff], axis=0))
    x1 = x[:,0,:,:,:].unsqueeze(1)
    x2 = x[:, 1, :, :, :].unsqueeze(1)
