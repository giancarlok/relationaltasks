...
def check(x,y):
    ...
    img = img.permute(0,2,3,1,4).reshape(batch_size,3, 32, 32*7)
    #print(img.shape)
    save_image(img, "same_diff6_hard.png")
...
def check_multi(x,y):
    ...
    batch_size = x.shape[0]
    img = torch.cat([x, torch.zeros_like(x[:, 0: 1])], dim=1)
    img[:,-1] += y.view(6*batch_size, 1, 1, 1)
...
def get_sample(batch_size, prob, shapes, colours):
    obj = np.stack([np.random.choice(len(shapes), size=2, replace=False) for _ in range(2*batch_size)], axis=0)
    x_aaa = np.stack([obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0]], axis=1)
    x_abb = np.stack([obj[:, 0], obj[:, 1], obj[:, 1], obj[:, 0], obj[:, 1], obj[:, 1]], axis=1)
    x_aba = np.stack([obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 0], obj[:, 1], obj[:, 0]], axis=1)

    obj = np.stack([np.random.choice(len(shapes), size=2, replace=False) for _ in range(batch_size)], axis=0)
    x_aaa_baa = np.stack([obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 0]], axis=1)
    x_baa_aaa = np.stack([obj[:, 1], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0]], axis=1)

    x_baa_aba = np.stack([obj[:, 1], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 1], obj[:, 0]], axis=1)
    x_aba_baa = np.stack([obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 0]], axis=1)

    x_aba_aaa = np.stack([obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0]], axis=1)
    x_aaa_aba = np.stack([obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 1], obj[:, 0]], axis=1)
    ...
    x = np.concatenate([x_aaa, x_abb, x_aba, x_aaa_baa, x_baa_aaa, x_baa_aba, x_aba_baa, x_aba_aaa, x_aaa_aba], axis=0)
    x = np.reshape(x, [-1])
    x = np.reshape(shapes[x], (12*batch_size, -1, 3, 32, 32))



    # x_same = np.expand_dims(x_same, axis=1)
    # print("x_same 4", x_same.shape)
    y = np.zeros(12*batch_size)
    y[-6*batch_size:] = 1
...
def get_test_sample(batch_size, prob, shapes, colours):
    obj = np.stack([np.random.choice(len(shapes), size=2, replace=False) for _ in range(6 * batch_size)], axis=0)
    x_aab = np.stack([obj[:, 0], obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 0], obj[:, 1]], axis=1)

    obj = np.stack([np.random.choice(len(shapes), size=2, replace=False) for _ in range(batch_size)], axis=0)
    x_aaa_aab = np.stack([obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 1]], axis=1)
    x_aab_aaa = np.stack([obj[:, 0], obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 0], obj[:, 0]], axis=1)

    x_aab_baa = np.stack([obj[:, 0], obj[:, 0], obj[:, 1], obj[:, 1], obj[:, 0], obj[:, 0]], axis=1)
    x_baa_aab = np.stack([obj[:, 1], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 1]], axis=1)

    x_aab_aba = np.stack([obj[:, 0], obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 1], obj[:, 0]], axis=1)
    x_aba_aab = np.stack([obj[:, 0], obj[:, 1], obj[:, 0], obj[:, 0], obj[:, 0], obj[:, 1]], axis=1)



    x = np.concatenate([x_aab, x_aaa_aab, x_aab_aaa, x_aab_baa, x_baa_aab, x_aab_aba, x_aba_aab], axis=0)
    x = np.reshape(x, [-1])
    x = np.reshape(shapes[x], (12*batch_size, -1, 3, 32, 32))
