...
def check(x,y):
	...
	img = img.permute(0,2,3,1,4).reshape(batch_size,3, 32, 32*10)
	save_image(img, "dist3_sc_colours.png")

def get_sample(batch_size, prob, shapes, colours):
	batch_size = 3*batch_size
	same = int(batch_size*prob[0])
	diff = batch_size-same
	x_shapes = np.array([np.random.choice(len(shapes), size=9, replace=True) for _ in range(batch_size)])
	# pick 4 colours
	obj= np.stack([np.random.choice(len(colours), size=4, replace=False) for _ in range(batch_size)],axis=0)
	# take the first 3 out of those chosen colours for positions 1,2 and 3.
	x_start =np.stack([obj[:,0], obj[:,1], obj[:,2]],axis=1)
	# permute those three colours and remove last one for positions 4 and 5
	...
	x_c = np.concatenate([x_start,x_perm, x_mc], axis=1)
	# print("x", x.shape)
