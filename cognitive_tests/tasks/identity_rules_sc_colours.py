...
def check(x,y):
	...
	img = img.permute(0,2,3,1,4).reshape(batch_size,3, 32, 32*10)
	save_image(img, "identity_rules_sc_colours.png")

def get_sample(batch_size, prob, shapes, colours):
	same = int(batch_size*prob[0])
	diff = batch_size-same
	x_shapes = np.array([np.random.choice(len(shapes), size=9, replace=True) for _ in range(3 * batch_size)])
	# AAA
	# pick 4 shapes

	obj = np.stack([np.random.choice(len(colours), size=4, replace=False) for _ in range(batch_size)], axis=0)
	# take the first one out of those chosen colours for positions 1,2 and 3 (since all of those are the same shape).
	x_aaa1 =np.stack([obj[:,0], obj[:,0], obj[:,0]],axis=1)
	# take another one out of the remaining colours for positions 4 and 5 (those should be the same too).
	...
	# pick 4 colours
	obj = np.stack([np.random.choice(len(colours), size=4, replace=False) for _ in range(batch_size)], axis=0)
	# take the first out of those chosen colours for positions 1,2 and 3.
	x_aba1 = np.stack([obj[:, 0], obj[:, 1], obj[:, 0]], axis=1)
	# take the first out of those chosen colours for positions 4 and 5.
	...
	x_c=np.concatenate([x_aaa,x_aba, x_abb], axis=0)
	x_shapes = np.reshape(x_shapes, [-1])
	x_shapes = np.reshape(shapes[x_shapes], (3 * batch_size, -1, 3, 32, 32))
